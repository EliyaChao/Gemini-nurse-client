...
// ✅ Gemini API 金鑰（請在 Render 環境變數設定 GEMINI_API_KEY）
...
app.post("/api/gemini", async (req, res) => {
    ...
    try {
        ...
        const generatedText =
            result.candidates?.[0]?.content?.parts?.[0]?.text?.trim() || "（沉默...）";

        res.json({ reply: generatedText });
    } catch (err) {
        console.error("Gemini API 錯誤:", err);
        res.status(500).json({
            reply: "（焦慮地環顧四周）別再問了！我想出院...",
        });
    }
});

/* -------------------------------------------------
   4️⃣ mock CRUD API（管理介面）
--------------------------------------------------- */
...
app.post("/mocks", (req, res) => {
    ...
    if (!message || !reply) {
        return res
            .status(400)
            .json({ success: false, message: "缺少 message 或 reply" });
    }

    const keywords = message.split(",").map((word) => ({
        word: word.trim(),
        weight: 1,
    }));
...
app.put("/mocks/:idx", (req, res) => {
    ...
    if (!keywords || !reply) {
        return res
            .status(400)
            .json({ success: false, message: "缺少 keywords 或 reply" });
    }
...
/* -------------------------------------------------
   5️⃣ 啟動伺服器（Render 兼容）
--------------------------------------------------- */
const PORT = process.env.PORT || 10000; // Render 預設會給 PORT
const HOST = "0.0.0.0"; // 必須綁定 0.0.0.0 才能被 Render 接收
...
// ✅ 防止 Render 超時（避免 502）
server.keepAliveTimeout = 120000; // 120 秒
